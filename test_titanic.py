...
def test_titanic(test_dir="test_history"):
    ...
    num_trial = 5
    he = HyperEnsamble(train,target[:TRAIN], accuracy_error_func,None, test_dir, trial=num_trial)
    he.find_best_models()
    ...
    # reusing the result
    he = HyperEnsamble(train, target[:TRAIN], accuracy_error_func, None, test_dir, trial=1, model_confs = [main.SvmConf])
    he.find_best_models()
